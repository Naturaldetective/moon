...
<template>
  <section flex-center text-14>
    <draggable
      class="nav w-auto flex flex-wrap justify-center gap-x-6 gap-y-2 w-90p sm:gap-x-12"
      :list="siteStore.data"
      item-key="id"
      :component-data="{
        tag: 'div',
    ...
    >
      <template #item="{ element: cate, index: i }: { element: Category, index: number }">
        <div
          ...
          cursor-pointer px-8 h-40 flex items-center transition-color duration-300
